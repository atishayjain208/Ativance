    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {MODULE_CARDS.map(({ title, description, color, border, iconBg, icon, badge, to }) => (
            <Link
              key={title}
              to={to}
              className={`relative bg-gradient-to-br ${color} border ${border} rounded-2xl p-5 flex flex-col gap-4 transition-transform duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-black/30 cursor-pointer block`}
            >
              {/* Badge */}
              <span className="absolute top-4 right-4 text-[10px] font-semibold uppercase tracking-wider text-emerald-400 bg-emerald-500/10 border border-emerald-500/25 rounded-full px-2 py-0.5">
                {badge}
              </span>
